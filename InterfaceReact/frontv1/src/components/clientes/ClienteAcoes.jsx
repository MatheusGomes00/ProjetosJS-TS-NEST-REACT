...
import { FaWhatsapp, FaEdit, FaTrash, FaBars } from "react-icons/fa";

export const ClienteAcoes = ({ cliente, onWhatsapp, onEdit, onDelete}) => {
            ...
            <div className="relative"
                ...
                {open && (
                    <div className={"absolute top-full left-1/2 -translate-x-1/2 mt-2 flex gap-3 bg-white border rounded-lg shadow p-2 z-10"}>
                        <button className="text-green-600 hover:text-green-800 relative group"
                                onMouseDown={(e) => {
                                    e.stopPropagation()
                                    onWhatsapp(cliente)
                                }}
